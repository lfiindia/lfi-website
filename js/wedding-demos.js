// ============ Wedding Demo Gallery — search, filter, render ============
(function () {
  ...
  function cardTemplate(item, catName, uniqueName) {
    ...
    const audioPreviewUrl =
      item.audioPreviewUrl || (isAudio ? `https://drive.google.com/uc?export=download&id=${item.id}` : "");
    const thumbContent = item.thumbnail
      ? `<img src="${item.thumbnail}" alt="${safeTitle} preview" loading="lazy" referrerpolicy="no-referrer" />`
      : `<div class="demo-thumb-fallback ${isAudio ? "audio" : ""}"><strong>${
          isAudio ? "Audio" : toInitials(uniqueName)
        }</strong><span>${safeDescription || "Preview"}</span></div>`;
    const audioPlayer = audioPreviewUrl
      ? `<audio class="demo-audio" controls preload="none" src="${audioPreviewUrl}">Audio preview is not supported in this browser.</audio>`
      : "";

    return `
      <article class="demo-card reveal visible" data-name="${safeTitle.toLowerCase()}" data-category="${catName}">
        <div class="demo-thumb" data-initial="${toInitials(uniqueName)}">
          ${thumbContent}
          <span class="demo-thumb-badge">${catName}</span>
          ${isAudio ? "" : '<span class="demo-play-dot" aria-hidden="true"></span>'}
        </div>
        <div class="demo-body">
          <p class="demo-title">${safeTitle}</p>
          ${safeDescription ? `<p class="demo-desc">${safeDescription}</p>` : ""}
          ${audioPlayer}
  ...
  function renderChips() {
    const chipList = [`<button class="filter-btn ${active === "all" ? "active" : ""}" data-filter="all">All Categories</button>`]
      .concat(
        categories.map(
          (cat) => `<button class="filter-btn ${active === cat.key ? "active" : ""}" data-filter="${cat.key}">${cat.short}</button>`
        )
      )
      .join("");
  ...
  function buildSection(cat) {
    ...
    return `
      <section class="demo-section" id="${cat.key}">
  ...
  function renderSections() {
    ...
    sectionsWrap.innerHTML =
      html || `<section class="demo-section"><p class="text-muted">No demos found for your search. Try another keyword.</p></section>`;
    ...
    sectionsWrap.querySelectorAll("img").forEach((img) => {
      img.addEventListener("error", () => {
        const wrapper = img.closest(".demo-thumb");
        if (!wrapper) return;
        img.remove();
        const initials = wrapper.getAttribute("data-initial") || "DE";
        const fallback = document.createElement("div");
        fallback.className = "demo-thumb-fallback";
        fallback.innerHTML = `<strong>${initials}</strong>`;
  ...
  // Jump straight to a category if the page was opened with #cat-N
  if (location.hash) {
